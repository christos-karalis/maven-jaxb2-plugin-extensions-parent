...
public abstract class AbstractFieldClone extends Plugin {
        ...
        for (ClassOutline co : outline.getClasses()) {
            ...
            while (iterator.hasNext()) {
                ...
                if (method.params().size()==1
                        ...
                        && method.params().get(0).type().name().equals(typeName)) {
                    ...
                    iterator.remove();
                        ...
                        && method.type().name().equals(typeName)
     ...
     * @param fieldType the returned byte[] type
     * @param getterMethods the getter methods that should be added
     * @param getterComments the comments to be added on the comments
     */
    protected void generateGetters(JDefinedClass jdc, List<JFieldVar> fields, JType fieldType, List<String> getterMethods, List<JDocComment> getterComments) {
        for (int i = 0 ; i < getterMethods.size() ; i ++) {
            String removedMethod = getterMethods.get(i);
            JMethod createdMethod = jdc.method(JMod.PUBLIC, fieldType, removedMethod);
            ...
            for (JFieldVar field : fields) {
                if (createdMethod.name().substring(3).equalsIgnoreCase(field.name())) {
                    createdMethod.body()._return(JOp.cond(JOp.ne(field, JExpr._null()), JExpr.cast(fieldType, JExpr.invoke(field, "clone")) , JExpr._null()));
                }
            }
        }
    }
     ...
     */
    protected void generateSetters(JDefinedClass jdc, List<JFieldVar> fields, JType voidType, JType fieldType, List<String> setterMethods, List<JDocComment> setterComments) {
        for (int i = 0 ; i < setterMethods.size() ; i ++) {
            ...
            JVar var = createdMethod.param(fieldType, "value");
            ...
            for (JFieldVar field : fields) {
                if (createdMethod.name().substring(3).equalsIgnoreCase(field.name())) {
                    createdMethod.body().assign(JExpr._this().ref(field), JOp.cond(JOp.ne(var, JExpr._null()), JExpr.cast(fieldType, JExpr.invoke(var, "clone")), JExpr._null()));
                }
            }
        }
    }
