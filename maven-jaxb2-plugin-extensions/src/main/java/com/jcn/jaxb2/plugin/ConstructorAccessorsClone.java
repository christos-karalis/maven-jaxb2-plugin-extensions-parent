...
public class ConstructorAccessorsClone extends Plugin {
    ...
    public boolean run(Outline outline, Options options, ErrorHandler errorHandler) throws SAXException {
        for (ClassOutline co : outline.getClasses()) {
            ...
            if (co.getSuperClass()==null) {
                reconstructConstructor(jdc, null, null);
            } else {
                ...
                while (superConstructors.hasNext()) {
                    ...
                    if (superConstructor.params().size()>0) {
                        ...
                        reconstructConstructor(jdc, superParameters, superConstructor);
                    }
                }
            }
     ...
     */
    private void reconstructConstructor(JDefinedClass jdc, JVar[] superParams, JMethod superConstructor) {
            ...
            for (JVar var : vars) {
                ...
                if (!existingOnSuper) {
                    if (var.type().name().equals("Date")
                        ...
                        //on byte[] or Date field, it creates a cloned copy
